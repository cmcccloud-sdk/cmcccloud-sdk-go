...
func TestEnvCredentialProvider_GetCredentials3(t *testing.T) {
	...
	err := os.Setenv("CMCCCLOUD_SDK_AK", MockAk)
	assert.Nil(t, err)
	err = os.Setenv("CMCCCLOUD_SDK_SK", MockSk)
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	credentials, err := p.GetCredentials()
	assert.Nil(t, err)
	...
	p = BasicCredentialEnvProvider()
	assert.Nil(t, err)
	err = os.Setenv("CMCCCLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err = p.GetCredentials()
	assert.Nil(t, err)
...
func TestEnvCredentialProvider_GetCredentials5(t *testing.T) {
	...
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	err = os.Setenv("CMCCCLOUD_SDK_IDP_ID", MockIdpId)
	assert.Nil(t, err)
	err = os.Setenv("CMCCCLOUD_SDK_ID_TOKEN_FILE", MockIdTokenFile)
	assert.Nil(t, err)
	err = os.Setenv("CMCCCLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err := p.GetCredentials()
...
func resetEnv() error {
	envs := []string{
		"CMCCCLOUD_SDK_AK",
		"CMCCCLOUD_SDK_SK",
		"CMCCCLOUD_SDK_PROJECT_ID",
		"CMCCCLOUD_SDK_DOMAIN_ID",
		"CMCCCLOUD_SDK_CREDENTIAL_TYPE",
		"CMCCCLOUD_SDK_IDP_ID",
		"CMCCCLOUD_SDK_ID_TOKEN_FILE",
	}
